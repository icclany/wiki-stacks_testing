...
var User = models.User;
 ...
 describe('http requests', function () {
  ...
  describe('POST /wiki', function () {
    ...
    xit('responds with 302', function(done) {
      return agent
      .post('/wiki')
    ...
    it('creates a page in the database', function(done) {
      return agent
      .post('/wiki')
      .send({name: 'Iris', email: 'iris@aol.com', title: 'My Test Page 2',tags:"", content: "stuff"})
      .end(function(err, res) {
        if (err) done(err);
        Page.find({title: 'My Test Page 2'}).then(function(page) {
